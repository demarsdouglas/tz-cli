...
var timeZones = []string{
	...
	"Antarctica/Macquarie", "Antarctica/Mawson", "Antarctica/Palmer", "Antarctica/Rothera", "Antarctica/Syowa",
	...
	"Asia/Dhaka", "Asia/Dili", "Asia/Dubai", "Asia/Dushanbe", "Asia/Famagusta", "Asia/Gaza", "Asia/Hebron", "Asia/Ho_Chi_Minh",
	"Asia/Hong_Kong", "Asia/Hovd", "Asia/Irkutsk", "Asia/Jakarta", "Asia/Jayapura", "Asia/Jerusalem", "Asia/Kabul", "Asia/Kamchatka",
	"Asia/Karachi", "Asia/Kathmandu", "Asia/Khandyga", "Asia/Kolkata", "Asia/Krasnoyarsk", "Asia/Kuala_Lumpur", "Asia/Kuching",
	...
	"Australia/Melbourne", "Australia/Perth", "Australia/Sydney", "Europe/Amsterdam", "Europe/Andorra", "Europe/Astrakhan",
...
func FindMatchingZone(input string) (string, error) {
	normInput := normalize(input)
	matches := []string{}

	for _, zone := range timeZones {
		if strings.Contains(normalize(zone), normInput) {
	...
	if len(matches) == 1 {
		...
		return "", fmt.Errorf("no timezone found matching: %s", input)
	} else {
		return "", fmt.Errorf("multiple matches found for '%s': %v", input, matches)
	}
}
